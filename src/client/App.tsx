...
import { Button,StateButton,Badge,Empty,Modal,ConfirmProvider,useConfirm,ThemeTogglerButton,ThemeProvider,useTheme } from './components/ui'
import {
  Search,
  RefreshCw,
  Plus,
...
function AppContent(){const { confirm } = useConfirm();const store=useWorkspace();const { theme, setTheme, toggleTheme } = useTheme();const [view,setView]=useState('Invoices');const [selected,setSelected]=useState('');const [query,setQuery]=useState('');const [filter,setFilter]=useState('all');const [client,setClient]=useState('');const [project,setProject]=useState('');const [from,setFrom]=useState('');const [to,setTo]=useState('');const [archived,setArchived]=useState(false);const [page,setPage]=useState(0);const [action,setAction]=useState<{name:string;invoice:Invoice}|null>(null);const [notice,setNotice]=useState('');const [undo,setUndo]=useState<Invoice|null>(null);const [email,setEmail]=useState('');const [signing,setSigning]=useState(false);const [refreshKey,setRefreshKey]=useState(0);const [sampleModalOpen,setSampleModalOpen]=useState(false);const [commandOpen,setCommandOpen]=useState(false);const [shortcutsOpen,setShortcutsOpen]=useState(false);const [emailDiagnosticsOpen,setEmailDiagnosticsOpen]=useState(false);const [selectedIds,setSelectedIds]=useState<Set<string>>(new Set());const [saveViewModalOpen,setSaveViewModalOpen]=useState(false);const [newViewName,setNewViewName]=useState('');const [savedViews,setSavedViews]=useState<SavedView[]>(()=>{try{return JSON.parse(localStorage.getItem('invoiceui:saved-views')||'[]')}catch{return []}});const [bulkResult,setBulkResult]=useState<{title:string;items:{id:string;name:string;status:'success'|'skipped'|'failed';reason?:string}[]}|null>(null);
 ...
 async function doAction(name:string,i:Invoice){try{if(name==='duplicate'){const id=crypto.randomUUID();await store.command({type:'duplicate',id:i.id,newId:id});setSelected(id);setView('Editor');setNotice('Duplicated as a fresh draft.')}else if(name==='archive'){await store.command({type:'archive',id:i.id,value:!i.archived});setUndo(i);setNotice(i.archived?'Invoice restored.':'Invoice archived.');setView('Invoices')}else if(name==='deleted'||name==='deleteDraft'){if(w?.invoices.some(inv=>inv.id===i.id)){await store.command({type:'deleteDraft',id:i.id})}setSelected('');setView('Invoices');setNotice('Draft invoice deleted.')}else setAction({name,invoice:i})}catch(e){setNotice(e instanceof Error?e.message:'Action failed')}}
 const saveCustomView=async()=>{if(!newViewName.trim())return;const nextView:SavedView={id:crypto.randomUUID(),name:newViewName.trim(),filter,client:client||undefined,project:project||undefined};const updated=[...savedViews,nextView];setSavedViews(updated);localStorage.setItem('invoiceui:saved-views',JSON.stringify(updated));await new Promise(r=>setTimeout(r,600));setNewViewName('');setSaveViewModalOpen(false);setNotice(`Saved view "${nextView.name}" created.`)};
 ...
 async function bulkDeleteDrafts(draftsToDelete:Invoice[]){if(!draftsToDelete.length)return;const ok=await confirm({title:'Delete draft invoices?',description:`Permanently delete ${draftsToDelete.length} selected draft invoice(s)? All draft changes will be discarded.`,confirmText:`Delete ${draftsToDelete.length===1?'draft':'drafts'}`,confirmVariant:'danger'});if(!ok)return;const items:{id:string;name:string;status:'success'|'skipped'|'failed';reason?:string}[]=[];for(const inv of draftsToDelete){const label=inv.number||`Draft (${inv.client.name||'Client'})`;try{await store.command({type:'deleteDraft',id:inv.id});localStorage.removeItem(`draft:${inv.id}`);items.push({id:inv.id,name:label,status:'success'})}catch(err){items.push({id:inv.id,name:label,status:'failed',reason:err instanceof Error?err.message:'Delete failed'})}}setBulkResult({title:'Bulk draft deletion results',items});setSelectedIds(new Set())}
 ...
 const emailIssueCount = w ? (w.messages || []).filter(m => ['failed', 'bounced'].includes(m.status)).length : 0
 ...
 return <MotionConfig reducedMotion="user"><div className="app-shell">{swUpdateWorker && (
 ...
 )}<header className="app-header"><div className="flex items-center gap-3"><button className="app-brand" onClick={()=>setView('Invoices')}><span>▤</span> InvoiceUI</button><div className="profile-switcher-container"><button type="button" className="profile-switcher-btn" onClick={()=>setProfileDropdownOpen(o=>!o)} aria-label="Switch business profile" title="Switch business profile"><span className="profile-dot" style={{backgroundColor:w.business.accent||'#863bff'}}/><span className="profile-name">{w.business.name||'Personal Workspace'}</span><span className="text-[10px] opacity-60">▼</span></button>{profileDropdownOpen&&<div className="profile-dropdown-menu"><div className="profile-dropdown-header">Trading Profiles</div>{(w.profiles||[]).map(p=>{const isCurrent=p.id===w.activeProfileId;const canDelete=!p.isDefault&&(w.profiles||[]).length>1;return <div key={p.id} className={`profile-dropdown-item flex items-center justify-between gap-1.5 ${isCurrent?'active':''}`}><button type="button" className="flex items-center gap-2 flex-1 text-left min-w-0" onClick={()=>void handleSwitchProfile(p.id)}><span className="profile-dot" style={{backgroundColor:p.business.accent||'#863bff'}}/><span className="flex-1 truncate">{p.name}</span>{p.isDefault&&<span className="text-[9px] opacity-50 uppercase">Default</span>}{isCurrent&&<span className="text-xs">✓</span>}</button>{canDelete&&!isCurrent&&<button type="button" className="text-rose-500 hover:text-rose-700 p-1 rounded opacity-60 hover:opacity-100 transition" title={`Delete profile "${p.name}"`} aria-label={`Delete profile ${p.name}`} onClick={async(e)=>{e.stopPropagation();const ok=await confirm({title:'Delete trading profile?',description:`Delete business profile "${p.name}"? Isolated documents and data for other profiles will not be affected.`,confirmText:'Delete profile',confirmVariant:'danger'});if(ok){try{await store.command({type:'deleteBusinessProfile',profileId:p.id});setNotice(`Business profile "${p.name}" deleted.`)}catch(err){setNotice(err instanceof Error?err.message:'Could not delete profile')}}}}><Trash2 size={12} animateOnHover /></button>}</div>})}<div className="profile-dropdown-divider"/><button type="button" className="profile-dropdown-item new-profile" onClick={()=>{setProfileDropdownOpen(false);setNewProfileModalOpen(true)}}><span>+</span><span>New business profile</span></button></div>}</div></div><AnimatedTabs tabs={['Invoices','Quotes','Attention','Clients','Projects','Services','Emails','Settings']} activeTab={view==='Editor'?'Invoices':view} onTabChange={tab=>{setView(tab);setNotice('')}}/><div className="header-actions"><button type="button" className="button ghost text-xs flex items-center gap-1.5" onClick={()=>setCommandOpen(true)} title="Global command palette (Ctrl+K)"><Search size={13} animateOnHover /><span>Search</span><span className="command-shortcut">Ctrl K</span></button><ThemeTogglerButton variant="ghost" size="sm" modes={['light','dark','system']} title="Toggle light, dark, or system theme" aria-label="Toggle editor theme" /><Button variant="ghost" onClick={()=>void store.refresh().then(()=>{setRefreshKey(k=>k+1);setNotice('Workspace refreshed. Unsaved edits are kept for recovery.')}).catch(e=>setNotice(e.message))}><RefreshCw size={12} animateOnHover className="mr-1 inline" />Refresh</Button><Button variant="ghost" onClick={()=>void store.logout().catch(e=>setNotice(e.message))}>Sign out</Button></div></header>{store.demo&&<div className="demo-banner">Local preview · saved in this browser · email and public links are unavailable</div>}<main className="workspace-main">{notice&&<div className="toast" role="status"><span>{notice}</span><div className="actions">{undo&&<Button onClick={()=>void store.command({type:'archive',id:undo.id,value:undo.archived}).then(()=>{setUndo(null);setNotice('Archive action undone.')})}>Undo</Button>}<Button variant="ghost" aria-label="Dismiss notification" onClick={()=>{setNotice('');setUndo(null)}}>×</Button></div></div>}
  ...
  <nav className="mobile-nav-bar" aria-label="Mobile navigation">
    ...
    <button type="button" className={`mobile-nav-item ${view==='Emails'?'active':''}`} onClick={()=>{setView('Emails');setNotice('')}}>
      <span className="mobile-nav-icon">✉</span>
      <span className="mobile-nav-label">Emails</span>
      {emailIssueCount>0&&<span className="mobile-nav-badge">{emailIssueCount}</span>}
    </button>
